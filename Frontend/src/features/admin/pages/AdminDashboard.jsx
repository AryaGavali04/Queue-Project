...
const AdminDashboard = () => {
  ...
  const fetchAll = async () => {
    ...
    await Promise.all([fetchDoctors(), fetchStaff(), fetchQueue()]);
    // Only fetch services if NOT hospital (categoryId !== 1)
    const categoryId = statsRes?.branchCategoryId;
    if (categoryId !== 1) {
      await fetchServices();
    }
  };
  ...
  const fetchStats = async () => {
    try {
      const r = await axios.get(`${API}/dashboard-stats`, { headers: authHeaders });
      setStats(r.data);
      return r.data;
    } catch(e) {
      return null;
  ...
  const fetchDoctors  = async () => {
    try {
      const r = await axios.get(`${API}/doctors`, { headers: authHeaders });
  ...
  const fetchServices = async () => {
    try {
      const r = await axios.get(`${API}/services`, { headers: authHeaders });
  ...
  const fetchStaff = async () => {
    try {
      const r = await axios.get(`${API}/staff`, { headers: authHeaders });
  ...
  const fetchQueue = async () => {
    try {
      const r = await axios.get(`${API}/queue/today`, { headers: authHeaders });
