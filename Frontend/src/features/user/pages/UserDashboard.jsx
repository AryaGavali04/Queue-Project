...
const UserDashboard = () => {
    ...
    const fetch_ = async () => {
      try {
        const [histRes, activeRes] = await Promise.all([
          fetch(`https://queue-project-1.onrender.com/api/v1/tokens/user/${userId}/history`, { headers: getAuthHeaders() }),
          fetch(`https://queue-project-1.onrender.com/api/v1/tokens/user/${userId}/active`,  { headers: getAuthHeaders() })
        ]);
  ...
  // ── Fetch categories ───────────────────────────────────
  useEffect(() => {
    fetch("https://queue-project-1.onrender.com/api/categories", { headers: getAuthHeaders() })
